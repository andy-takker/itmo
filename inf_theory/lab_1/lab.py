...
def decode_freq(encoded_str: bytes) -> Dict[bytes, int]:
...
def decode_huffman(code_table: Dict[bytes, str], binary_string: str) -> bytes:
...
def bytes_to_str(byte_str: bytes) -> str:
    ...
    string_length = len(byte_str) * 8
    return ("{:0"+str(string_length)+"b}").format(n)
